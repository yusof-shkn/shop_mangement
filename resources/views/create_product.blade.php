...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div class="container bg-light w-25 p-4 position-fixed" style="border-radius: 1rem; top:50%;left:50%;transform:translate(-50%,-50%)">
        ...
        <form method="POST" action="{{ route('inventory_manager.create') }}">
            @csrf
            ...
            <div class="mt-4 adminsInput" >
                ...
                <x-input id="price" class="block mt-1 w-full" type="number"  name="price" :value="old('price')" autocomplete="off" />
            </div>
            <div class="mt-4 adminsInput">
                <x-label for="quantity" value="{{ __('Product Quantity') }}" />
                <x-input id="quantity" class="block mt-1 w-full" type="number" name="quantity" :value="old('quantity')" autocomplete="off" />
            </div>
